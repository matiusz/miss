...
from agents.Agents import AbstractAgent
...
class PolicyGradient:
    ...
    def choose_action(self, observation):
        ...
        for ob in observation:
            if isinstance(ob, (np.ndarray)):
                ob = ob[()]
            newobs.append(ob)
        observation = np.array(newobs).reshape(1, len(observation))
    ...
    def _discount_and_norm_rewards(self):
        ...
        discounted_ep_rs = discounted_ep_rs.astype('float64')
        discounted_ep_rs -= np.mean(discounted_ep_rs)
        if not np.std(discounted_ep_rs)==0:
